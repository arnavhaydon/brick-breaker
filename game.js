...
let ballRadius = 10
let paddleheight = 10 
let paddlewidth = parseInt(sessionStorage.getItem("paddle_width")) || 90
let score = 0
...
let brickRowCount = parseInt(sessionStorage.getItem("brick_rows")) || 3
...
let interval = setInterval(draw,parseInt(sessionStorage.getItem("ball_speed")) || 10)
    ...
    for(let p = 0 ;p <brickColumnCount ; p = p + 1){
        for(let f = 0 ;f <brickRowCount ; f = f + 1){
            if(bricks[p][f].status === 1){
                ...
                context.fillStyle = sessionStorage.getItem("colour") || "black"
                context.fill()
                context.closePath()
            }
